            ...
            for epoch in range(1, args.correct_epochs + 1):  
                ...
                if epoch<=int(args.correct_epochs*3/4):
                   for batch_idx, (data, target) in enumerate(train_loader):
                     ...
                     adjust_pred=confmodel(output, target, teacher_forcing_ratio=0)
                     ...
                     loss = maeloss(adjust_pred, label)
                     rloss.append(loss)
                     loss.backward()
                     optimizer.step()
                   print('**********Correction Epoch {}: Loss (prediction loss): {:.6f}'.format(epoch, loss))                     
                else:
                   T = len(y_true)
                   rloss=[]
                   predtest=int(T/3)
...
def train_cal_correction_CQR(model,args,data_loader,scaler,conf_correct_model='mlp',tinit=300):
            ...
            torch.save(confmodel_m.state_dict(), './model/saved_model/correction_m_{}'.format(args.save_filename))
            torch.save(confmodel_u.state_dict(), './model/saved_model/correction_u_{}'.format(args.save_filename))
            torch.save(confmodel_l.state_dict(), './model/saved_model/correction_l_{}'.format(args.save_filename))
            print("correction model saved!")
