...
if args.mode == 'train':
 ...
 # Save the model
    ...
    #correctmodel.load_state_dict(torch.load('./model/saved_model/correction_t{}_{}'.format(args.tinit, args.save_filename),map_location=args.device))
    correctmodel.load_state_dict(
        torch.load('./model/saved_model/correction_mlp_{}_{}'.format(args.tinit,args.save_filename),
    ...
    correctmodel.load_state_dict(torch.load( './model/saved_model/correction_gnn_{}_{}'.format(args.tinit,args.save_filename) ,map_location=args.device))
    model.load_state_dict(torch.load('./model/saved_model/{}'.format(args.save_filename),map_location=args.device))
    print("Start testing: Load saved model")
    trainer.test(model, correctmodel,trainer.args, test_loader, scaler, trainer.logger)
